...
const { generator, downloadRepo, logger, spinner } = require('./utils/index');
...
async function create(projectName, options) {
    ...
    // 是否从远程仓库获取模板
    if (options.repo) {
        const repoSpinner = spinner.create('正在从远程仓库下载模板...');
        repoSpinner.start();
        try {
            const branch = options.branch || 'main';
            await downloadRepo(options.repo, targetDir, branch);
            repoSpinner.succeed('远程模板下载成功');
            return;
        } catch (error) {
            repoSpinner.fail(`远程模板下载失败: ${error.message}`);
            return;
        }
    }
    ...
    // 生成项目
    const templateSpinner = spinner.create('正在生成项目...');
    ...
    try {
        ...
        if (!fs.existsSync(templateDir)) {
            templateSpinner.fail(`模板 "${templateName}" 不存在`);
            return;
        }

        await generator.generate(templateDir, targetDir, metadata);

        templateSpinner.succeed('项目创建成功!');
        ...
        templateSpinner.fail(`项目创建失败: ${error.message}`);
        logger.error(error);
    }
}
